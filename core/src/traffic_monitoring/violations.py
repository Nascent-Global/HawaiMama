...
from .domain import FrameContext, HelmetState, PlateState, TrafficClass, TrackState
...
def evaluate_track_violations(
    ...
    *,
    ...
    """Evaluate rule-based violations for a single track."""
...
class ViolationEngine:
    ...
    def _wrong_lane_track_ids(self, tracks: Sequence[TrackState]) -> set[int]:
        ...
        for track in tracks:
            ...
            if any(_point_in_polygon(center, polygon) for polygon in self.config.wrong_lane.lane_polygons):
                violating.add(track.track_id)
        return violating
